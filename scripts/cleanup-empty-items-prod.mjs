...
if (!MONGODB_URI) {
  ...
  console.error(
    "💡 Make sure .env.local is configured with production MongoDB URI"
  );
  process.exit(1);
}
  ...
  try {
    ...
    for (const userData of users) {
      ...
      if (userData.blocks) {
        for (const timeBlock of userData.blocks) {
          if (timeBlock.todos) {
            ...
            if (removedCount > 0) {
              console.log(
                `  📝 TimeBlock "${timeBlock.label}": Removed ${removedCount} empty todos`
              );
      ...
      if (userUpdated) {
        await userData.save();
        console.log(
          `  💾 Saved updates for production user ${userData.userId} (${userRemovedItems} items removed)`
        );
        totalUpdatedUsers++;
    ...
    if (totalUpdatedUsers > 0) {
      ...
      console.log(
        "🌐 Production site should now show correct remaining counts."
      );
    } else {
    ...
    console.error(
      "💡 Make sure you have deployed the latest code to production first"
    );
    process.exit(1);
  } finally {
