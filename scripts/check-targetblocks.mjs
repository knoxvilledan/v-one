...
async function checkTargetBlocks() {
  try {
    ...
    const docsWithTargetBlocks = await UserData.find({
      ...
      .select(
        "date userId masterChecklist.targetBlock habitBreakChecklist.targetBlock workoutChecklist.targetBlock todoList.targetBlock"
      )
      .lean();

    console.log(
      `📊 Found ${docsWithTargetBlocks.length} documents with targetBlock data`
    );
    ...
    if (docsWithTargetBlocks.length > 0) {
      ...
      docsWithTargetBlocks.slice(0, 5).forEach((doc, i) => {
        ...
        [
          ...
          if (list) {
            const targetBlocks = list
              .filter((item) => item.targetBlock !== undefined)
              .map((item) => item.targetBlock);
      ...
      console.log(
        `\n📈 All unique targetBlock values found: [${Array.from(
          allTargetBlocks
        )
          .sort((a, b) => a - b)
          .join(", ")}]`
